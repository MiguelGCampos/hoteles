...
public class GlobalHandlerException {
    ...
    // --- 409: Dato único duplicado / entidad relacionada ---
    @ExceptionHandler(ArgumentoNoValidoException.class)
    public ResponseEntity<CustomErrorResponse> handleArgumentoNoValido(ArgumentoNoValidoException ex) {
        log.warn("Argumento no válido (409): {}", ex.getMessage());
        return buildErrorResponse(HttpStatus.CONFLICT, ex.getMessage());
    }

    @ExceptionHandler(EntidadRelacionadaException.class)
    public ResponseEntity<CustomErrorResponse> handleEntidadRelacionada(EntidadRelacionadaException ex) {
        log.warn("Entidad relacionada (409): {}", ex.getMessage());
        return buildErrorResponse(HttpStatus.CONFLICT, ex.getMessage());
    }

    // --- 409: Estado inconsistente de la entidad (p.ej. ya eliminada) ---
    ...
    // --- 400: Argumento inválido no capturado por Bean Validation ---
